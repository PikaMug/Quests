...
public class Quester {
    ...
    public boolean hasCustomObjective(Quest quest, String s) {
        ...
        final Stage questStage = getCurrentStage(quest);

        if (questData == null || questStage == null) {
            return false;
        }

        if (questData.customObjectiveCounts.containsKey(s)) {
            int count = questData.customObjectiveCounts.get(s);
            int index = -1;
            for (int i = 0; i < getCurrentStage(quest).customObjectives.size(); i++) {
                if (questStage.customObjectives.get(i).getName().equals(s)) {
                    index = i;
                    break;
                }
            }
            int count2 = questStage.customObjectiveCounts.get(index);
            return count <= count2;
        }
        return false;
    }
    ...
    public void interactWithNPC(Quest quest, NPC n) {
        ...
        if (questData.citizensInteracted.containsKey(n.getId())) {
            if (!questData.citizensInteracted.get(n.getId())) {
                questData.citizensInteracted.put(n.getId(), true);
