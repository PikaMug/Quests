...
public class FabricQuestStageMenuPrompt extends FabricQuestsEditorIntegerPrompt {

    private final @NotNull UUID uuid;
    private final FabricQuestsPlugin plugin;
    private final int size = 2;

    public FabricQuestStageMenuPrompt(final @NotNull UUID uuid) {
        super(uuid);
        this.uuid = uuid;
    ...
    public String getTitle() {
        return FabricLang.get("stageEditorStages");
    }
    ...
    public String getSelectionText(final int number) {
        ...
        if (number > 0) {
            if (number < stages + 1) {
                return ChatFormatting.GOLD + FabricLang.get("stageEditorEditStage") + " " + number;
            } else if (number == stages + 1) {
                return ChatFormatting.YELLOW + FabricLang.get("stageEditorNewStage");
            } else if (number == stages + 2) {
    ...
    public void acceptInput(final Number input) {
        ...
        if (i > 0) {
            if (i < (stages + 1)) {
                new FabricQuestStageMainPrompt((i), uuid).start();
            } else if (i == (stages + 1)) {
                new FabricQuestStageMainPrompt((stages + 1), uuid).start();
            } else if (i == (stages + 2)) {
