...
public class FabricQuestAbandonPrompt extends FabricMiscStringPrompt {

    private final @NotNull UUID uuid;
    private final FabricQuestsPlugin plugin;

    public FabricQuestAbandonPrompt(final @NotNull UUID uuid, FabricQuestsPlugin plugin) {
        super(uuid);
        this.uuid = uuid;
        this.plugin = plugin;
    }
    ...
    public void acceptInput(final String input) {
        ...
        if (input.equalsIgnoreCase("1") || input.equalsIgnoreCase("y")
            ...
            final me.pikamug.quests.player.Quester quester = plugin.getQuester(player != null ? player.getUUID() : uuid);
            if (quester == null) {
                plugin.getPluginLogger().info("Ended conversation because quester for " + getName() + " was null");
                return;
            }
            final String questIdToQuit = quester.getQuestIdToQuit();
            try {
                final Quest quest = getQuestById(questIdToQuit);
                if (quest == null) {
                    plugin.getPluginLogger().info((player != null ? player.getName().getString() : uuid.toString())
                            + " attempted to quit quest ID \"" + questIdToQuit + "\" but something went wrong");
                    if (player != null) {
                    ...
                    final String msg = ChatFormatting.YELLOW + FabricLang.get("questQuit").replace("<quest>",
                            ChatFormatting.DARK_PURPLE + quest.getName() + ChatFormatting.YELLOW);
            ...
            new FabricQuestAbandonPrompt(player != null ? player.getUUID() : uuid, plugin).start();
        }
    }
