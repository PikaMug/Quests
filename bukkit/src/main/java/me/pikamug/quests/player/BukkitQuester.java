...
public class BukkitQuester implements Quester {
        ...
        for (final Objective obj : q.getCurrentObjectives(quest, false, false)) {
            ...
            if (!ignoreOverrides && objIndex < overrides.size()) {
                ...
                String message = (!s.trim().isEmpty() ? "- " : "") + ChatColor.GREEN + BukkitConfigUtil
                        .parseString(s, quest, q.getPlayer());
                message = message.replace("<count>", objective.getProgress() + "/" + objective.getGoal());
                message = message.replace("%count%", objective.getProgress() + "/" + objective.getGoal());
                if (plugin.getDependencies().getPlaceholderApi() != null) {
                    message = PlaceholderAPI.setPlaceholders(q.getPlayer(), message);
                }
                quester.sendMessage(message);
                continue;
            }
            String message = "- " + BukkitLang.BukkitFormatToken.convertString(q.getPlayer(),
            ...
            if (objective.getGoalAsBlockItem() != null) {
                ...
                if (localeManager != null && settings.canTranslateNames()) {
                    localeManager.sendMessage(q.getPlayer(), message, goal.getType(), goal.getDurability(), null);
                }
                ...
                if (localeManager != null && settings.canTranslateNames() && !goal.hasItemMeta()
                    ...
                    localeManager.sendMessage(q.getPlayer(), message, goal);
                ...
                if (localeManager != null && settings.canTranslateNames()) {
                    localeManager.sendMessage(q.getPlayer(), message, goal.getEntityType(), null);
                } else {
    ...
    public void breakBlock(final Quest quest, final BlockItemStack broken) {
        ...
        if (progress == goal.getAmount()) {
            ...
            dispatchMultiplayerObjectives(quest, getCurrentStage(quest), (final Quester q) -> {
                ...
                ((BukkitQuester) q).finishObjective(quest, new BukkitObjective(type, null, progress, finalGoal), null, null, null,
                        null, null, null, null);
                return null;
            });
        }
        ...
        for (final Integer match : matches) {
            ...
            if (progress >= goal.getAmount()) {
                ...
                dispatchMultiplayerObjectives(quest, getCurrentStage(quest), (final Quester q) -> {
                    ...
                    ((BukkitQuester) q).finishObjective(quest, new BukkitObjective(type, null, progress, goal), null, null, null,
                            null, null, null, null);
                    return null;
                });
            }
    ...
    public void finishObjective(final Quest quest, final Objective objective, final EntityType mob,
        ...
        dispatchMultiplayerObjectives(quest, getCurrentStage(quest), (final Quester q) -> {
            ((BukkitQuester) q).finishObjective(quest, objective, mob, extra, npc, location, color, pass, co);
            return null;
        });
